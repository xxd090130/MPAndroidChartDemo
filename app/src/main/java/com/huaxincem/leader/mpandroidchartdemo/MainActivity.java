package com.huaxincem.leader.mpandroidchartdemo;
...
//        setChartStyle(chart, mLineData, Color.WHITE);
    ...
    private void test(LineChart lineChart) {
        initLineChart(lineChart);

        List<Entry> entriesY = new ArrayList<>();

        for (int i = 0; i < 5; i++) {
            entriesY.add(new Entry(i, 3*i));
        }

        LineDataSet lineDataSetY = new LineDataSet(entriesY, "Y轴");
        LineData lineData = new LineData(lineDataSetY);
